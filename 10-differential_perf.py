...
for name, livetime in sorted(livetimes.items()):
    ...
    # Build the models with the full MC
...
out = {
    ...
    "mu2flux": multiinj.mu2flux(1.),
    "mu2flux_per_src": multiinj.mu2flux(1., per_source=True).tolist(),
    "rnd_seed": rnd_seed,
    "ntrials": ntrials,
    "gamma_inj": gamma_inj,
    "E0_inj": E0_inj,
...
_fname = os.path.join(
    outpath,
    "mu={:05.1f}_lo={:04.2f}_hi={:04.2f}.json.gz".format(mu,
                                                         log_E_nu_lo,
                                                         log_E_nu_hi)
    )
